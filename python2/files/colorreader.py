...
class ColorReaderFrame(ttk.Frame):
...
class ColorsTextFrame(ttk.Frame):
    def __init__(self, *args, **kwargs):
        ttk.Frame.__init__(self, *args, **kwargs)
    ...
    def create_scrollbar(self):
        self.scroll = ttk.Scrollbar(self, orient=VERTICAL, command=self.coloredtext.yview)
...
class ColorsBottomOptionsFrame(ttk.Frame):
    def __init__(self, *args, **kwargs):
        ttk.Frame.__init__(self, *args, **kwargs)
        ...
        self.palette_options_frame = ttk.LabelFrame(self, text="Colors", padding=3)
        self.palette_options_frame.columnconfigure(0, weight=1)
        self.palette_options_frame.rowconfigure(0, weight=0)

        # palette
        self.palette = ttk.Combobox(self.palette_options_frame, takefocus=False, state='readonly', values=('Purple & Teal', 'Red & Blue', ' ', ' '), textvariable=self.parent.parent.palette_var, width=13)
        self.palette.bind('<<ComboboxSelected>>', self.parent.parent.palette_click)
        self.palette.grid(column=0, row=0)

        # color pattern frame
        self.color_pattern_frame = ttk.LabelFrame(self, text="Color pattern", padding=3)
        ...
        self.color_item = ttk.Combobox(self.color_pattern_frame, textvariable=self.color_item_var, takefocus=False, state='readonly', values=('Words','Letters', ' ', ' '), width=7)
        ...
        self.color_order = ttk.Combobox(self.color_pattern_frame, textvariable=self.color_order_var, takefocus=False, state='readonly', values=('Ordered','Random', " ", ' '), width=8)
        ...
        self.anchor_option = ttk.Checkbutton(self.color_pattern_frame, variable=self.anchor_option_var, takefocus=False, text='Anchor color', command=self.anchor_color_click)
        self.anchor_option.grid(column=2, row=0)

        # font options frame
        self.font_options_frame = ttk.LabelFrame(self, text="Font", padding=3)
        ...
        # font combobox
        self.font_chooser = ttk.Combobox(self.font_options_frame, textvariable=self.parent.parent.font_chooser_var, takefocus=False, values=('Courier', 'Helvetica', 'Times', ' '), width=9, state="readonly")
        ...
        self.font_bold = ttk.Checkbutton(self.font_options_frame, text="Bold", variable=self.parent.parent.font_bold_var, command=self.parent.parent.font_bold_click, takefocus=False)
        self.font_bold.grid(column=2, row=0)

        # previous/next page frame
        self.prevnextframe = ttk.LabelFrame(self, padding=3, text="Page")
        ...
        self.prevbutton = ttk.Button(self.prevnextframe, text="<-", width=3, takefocus=False, command=self.parent.parent.on_prev_press)
        self.button_label = ttk.Label(self.prevnextframe, textvariable=self.current_page)
        self.nextbutton = ttk.Button(self.prevnextframe, text="->", width=3, takefocus=False, command=self.parent.parent.on_next_press)
