...
class Paster(Toplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.scroll = ttk.Scrollbar(self, orient=VERTICAL, command=self.text_box.yview)
        self.text_box.configure(yscrollcommand=self.scroll.set)

        self.button_frame = ttk.Frame(self)
        self.finished_button = ttk.Button(self.button_frame, text="Finished", takefocus=False, command=self.enter_text)
        self.cancel_button = ttk.Button(self.button_frame, text="Cancel", takefocus=False, command=self.destroy)
        self.blank = ttk.Label(self.button_frame, text="   ")
